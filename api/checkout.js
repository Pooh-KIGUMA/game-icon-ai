...
function cookieSecret() {
  return process.env.CREDIT_COOKIE_SECRET || process.env.SUPABASE_SECRET_KEY || process.env.SUPABASE_SERVICE_ROLE_KEY || 'iconia-credit-secret';
}
...
function setCookie(res, id) {
  res.setHeader('Set-Cookie', `${cookieName}=${encodeURIComponent(`${id}.${sign(id)}`)}; Path=/; Max-Age=31536000; HttpOnly; Secure; SameSite=Lax`);
}
...
export default async function handler(req, res) {
  ...
  if (!secret) return send(res, 503, { error: 'STRIPE_SECRET_KEY is not configured.' });

  const body = req.method === 'GET' ? (req.query || {}) : (req.body || {});
  const type = body.type === 'subscription' ? 'subscription' : 'credits';
  const key = String(body.product || body.pack || body.plan || '');
  const product = (type === 'subscription' ? PLANS : PACKS)[key];
  ...
  try {
    ...
    const result = await stripeCheckout(params, secret);
    if (!result.ok) {
      console.error('Stripe checkout error', result.status, result.data?.error || result.data);
      return send(res, 502, { error: result.data?.error?.message || 'Stripe checkout failed.', stripeStatus: result.status });
    }
    ...
    if (error?.name === 'AbortError') return send(res, 504, { error: 'Stripeへの接続がタイムアウトしました。もう一度お試しください。' });
    return send(res, 503, { error: 'CHECKOUT_SERVICE_UNAVAILABLE', detail: error?.message || 'unknown' });
  }
}
