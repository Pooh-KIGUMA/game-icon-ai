...
async function stripeCheckout(params, secret) {
  ...
  try { const r = await fetch('https://api.stripe.com/v1/checkout/sessions', { method:'POST', headers:{ Authorization:`Bearer ${secret}`, 'Content-Type':'application/x-www-form-urlencoded' }, body:params, signal:controller.signal }); const data = await r.json().catch(() => ({})); return { ok:r.ok, status:r.status, data }; } finally { clearTimeout(timer); }
}
export default async function handler(req, res) {
  ...
  try {
    ...
    const result = await stripeCheckout(params, secret); if (!result.ok) { console.error('Stripe checkout error', result.status, result.data?.error || result.data); return send(res, 502, { error:result.data?.error?.message || 'Stripe checkout failed.', stripeStatus:result.status }); }
    if (!result.data?.url) return send(res, 502, { error:'Stripe did not return a checkout URL.' });
    setCookie(res, userId);
    if (String(body.json || '') === '1') return send(res, 200, { ok:true, url:result.data.url }, { 'Cache-Control':'no-store' });
    return res.redirect(303, result.data.url);
