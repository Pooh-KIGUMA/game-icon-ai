...
function decodeCookie(value) {
  ...
  try { return crypto.timingSafeEqual(Buffer.from(m[2], 'hex'), Buffer.from(expected, 'hex')) ? m[1] : null; } catch { return null; }
}
async function stripe(url, options = {}) {
...
async function supabase(url, key, path, options = {}) {
  return fetch(`${url}/rest/v1/${path}`, { ...options, headers: { apikey:key, 'Content-Type':'application/json', ...(options.headers || {}) } });
}
...
export default async function handler(req, res) {
  ...
  try {
    ...
    for (const session of matches) {
      ...
      if (!profile) {
        const createRes = await supabase(supabaseUrl, serviceKey, 'profiles?on_conflict=id', { method:'POST', headers:{ Prefer:'resolution=ignore-duplicates,return=minimal' }, body:JSON.stringify({ id:userId,plan:'free',credits:3,monthly_credits:3,monthly_remaining:3,purchased_credits:0,bonus_credits:0 }) });
        if (!createRes.ok && createRes.status !== 409) throw new Error(`PROFILE_CREATE_FAILED:${await createRes.text()}`);
      }

      const purchaseRes = await supabase(supabaseUrl, serviceKey, 'credit_purchases?on_conflict=stripe_session_id', { method:'POST', headers:{ Prefer:'resolution=ignore-duplicates,return=representation' }, body:JSON.stringify({ stripe_session_id:session.id,user_id:userId,pack_id:session.metadata.product,credits,amount_jpy:product.amount }) });
      if (!purchaseRes.ok) throw new Error(`PURCHASE_RECORD_FAILED:${await purchaseRes.text()}`);
      const rows = await purchaseRes.json().catch(() => []);
      if (Array.isArray(rows) && rows.length) {
        const current = profile || { credits:3, purchased_credits:0 };
        const patchRes = await supabase(supabaseUrl, serviceKey, `profiles?id=eq.${encodeURIComponent(userId)}`, { method:'PATCH', headers:{ Prefer:'return=minimal' }, body:JSON.stringify({ credits:Number(current.credits||0)+credits,purchased_credits:Number(current.purchased_credits||0)+credits,updated_at:new Date().toISOString() }) });
        if (!patchRes.ok) throw new Error(`PROFILE_UPDATE_FAILED:${await patchRes.text()}`);
        added += credits;
      }
    }
