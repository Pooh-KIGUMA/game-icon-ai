...
function textStyle(message, width, height) {
  ...
  const palettes = {
    esports: ["#FFFFFF", "#6BC7FF", "#5A35FF"],
    luxury: ["#FFF7C7", "#F2B63D", "#7A470A"],
    cyber: ["#EFFFFF", "#28DFFF", "#7447FF"],
    japanese: ["#FFFFFF", "#FF4A4A", "#7A0714"],
    cute: ["#FFF5FC", "#FF79C6", "#8D63FF"],
    gothic: ["#FFFFFF", "#B79BFF", "#29134F"],
    minimal: ["#FFFFFF", "#D9E2EC", "#657487"],
    metal: ["#FFFFFF", "#B8C4D0", "#3E4855"],
  };
...
async function addExactText(buffer, message) {
  ...
  const outer = Math.max(5, Math.round(s.size * 0.06));
  ...
  const decoLeft = Math.max(0, s.x - s.size * 1.45);
  const decoRight = Math.min(width, s.x + s.size * 1.45);
  const decorative = s.style === "esports" ? `<path d="M ${decoLeft} ${s.y + s.size * .18} L ${decoLeft + s.size * .32} ${s.y + s.size * .03} L ${decoLeft + s.size * .58} ${s.y + s.size * .18} M ${decoRight} ${s.y + s.size * .18} L ${decoRight - s.size * .32} ${s.y + s.size * .03} L ${decoRight - s.size * .58} ${s.y + s.size * .18}" stroke="${s.accent}" stroke-width="${Math.max(3, s.size * .018)}" fill="none" opacity="0.9"/>` : s.style === "japanese" ? `<path d="M ${decoLeft} ${s.y + s.size * .25} L ${decoRight} ${s.y + s.size * .25}" stroke="${s.accent}" stroke-width="${Math.max(4, s.size * .025)}" opacity="0.85"/>` : s.style === "cyber" ? `<path d="M ${decoLeft} ${s.y + s.size * .18} L ${decoRight} ${s.y + s.size * .18}" stroke="${s.accent}" stroke-width="${Math.max(3, s.size * .018)}" opacity="0.9"/>` : s.style === "luxury" ? `<path d="M ${decoLeft} ${s.y + s.size * .22} L ${decoRight} ${s.y + s.size * .22}" stroke="${s.accent}" stroke-width="${Math.max(2, s.size * .012)}" opacity="0.75"/>` : "";
  const svg = `<svg width="${width}" height="${height}" xmlns="http://www.w3.org/2000/svg"><defs>${defs}</defs><g transform="${transform}" filter="${filter}">${decorative}<text ${common} fill="url(#${id}grad)" stroke="${stroke}" stroke-width="${outer}">${safe}</text><text ${common} fill="url(#${id}grad)" stroke="#FFFFFF" stroke-opacity="0.28" stroke-width="${inner}">${safe}</text><text ${common} fill="url(#${id}shine)" stroke="none" opacity="0.52">${safe}</text></g></svg>`;
  return sharp(buffer).composite([{ input: Buffer.from(svg), top: 0, left: 0 }]).jpeg({ quality: 94 }).toBuffer();
}

function buildPrompt({ message, history, hasImage, mode }) {
  const recent = Array.isArray(history)
    ? history.slice(-8).map(x => `${x.role === "user" ? "USER" : "ASSISTANT"}: ${clean(x.text, 1200)}`).join("\n")
    : "";
  const requestedText = extractRequestedText(message);
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const requestedText = extractRequestedText(message);
    const prompt = buildPrompt({ message, history, hasImage: Boolean(image), mode });
    let outputBuffer;
    ...
    if (image) {
      ...
      if (mode === "TEXT_ONLY") {
        ...
        if (requestedText) outputBuffer = await addExactText(outputBuffer, message);
      }
    } else {
