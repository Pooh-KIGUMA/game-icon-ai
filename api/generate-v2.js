...
function extractText(message) {
  ...
  const quoted = t.match(/[「『“]([A-Za-z0-9._-]{1,40})[」』”]/);
  if (quoted) return quoted[1];
  const patterns = [
    /(?:文字|テキスト|ロゴ|名前|チーム名|クラン名|同盟名)\s*(?:は|を|：|:|=)\s*[「『“]?([A-Za-z0-9._-]{1,40})/i,
    /([A-Za-z][A-Za-z0-9._-]{0,39})\s*(?:の文字|という文字)/i,
    /([A-Za-z][A-Za-z0-9._-]{0,39})\s*だけ(?:を|に)?/i,
    /([A-Za-z][A-Za-z0-9._-]{0,39})\s*(?:を入れて|を追加して|を入れたい|を入れてください)/i,
  ];
...
function isTextOverlayRequest(message, image) {
  ...
  return /文字|テキスト|ロゴ|名前|チーム|クラン|同盟|原画|元画像|そのまま|だけ|のみ|追加|入れ|入れて|クラン風|ロゴ風/i.test(t);
}
...
function makeLogoSvg(text, width, height, message) {
  ...
  const size = Math.max(58, Math.round(min * 0.105));
  let x = width / 2;
  ...
  return `<svg width="${width}" height="${height}" xmlns="http://www.w3.org/2000/svg"><defs><linearGradient id="${id}" x1="0" y1="0" x2="0" y2="1"><stop offset="0" stop-color="${c1}"/><stop offset=".48" stop-color="${c2}"/><stop offset="1" stop-color="${c3}"/></linearGradient><filter id="s" x="-40%" y="-40%" width="180%" height="220%"><feDropShadow dx="0" dy="${shadow}" stdDeviation="${Math.round(size*.035)}" flood-color="#000" flood-opacity=".9"/></filter></defs><g filter="url(#s)" transform="rotate(-3 ${x} ${y})"><path d="M ${Math.max(10,x-size*1.8)} ${y+size*.18} L ${Math.max(10,x-size*1.45)} ${y-size*.02} L ${Math.max(10,x-size*1.15)} ${y+size*.18} M ${Math.min(width-10,x+size*1.8)} ${y+size*.18} L ${Math.min(width-10,x+size*1.45)} ${y-size*.02} L ${Math.min(width-10,x+size*1.15)} ${y+size*.18}" fill="none" stroke="${accent}" stroke-width="${Math.max(3,size*.018)}"/><text x="${x}" y="${y}" text-anchor="${anchor}" font-family="Impact, Haettenschweiler, Arial Black, sans-serif" font-size="${size}px" font-weight="900" letter-spacing="${Math.round(size*.015)}px" fill="url(#${id})" stroke="#080A12" stroke-width="${stroke}" stroke-linejoin="round" paint-order="stroke">${safe}</text><text x="${x}" y="${y}" text-anchor="${anchor}" font-family="Impact, Haettenschweiler, Arial Black, sans-serif" font-size="${size}px" font-weight="900" letter-spacing="${Math.round(size*.015)}px" fill="none" stroke="#FFFFFF" stroke-opacity=".35" stroke-width="2" paint-order="stroke">${safe}</text></g></svg>`;
}
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const out = await sharp(source).composite([{ input: Buffer.from(svg), top:0, left:0 }]).jpeg({ quality:95 }).toBuffer();
    return res.status(200).json({ success:true, image:`data:image/jpeg;base64,${out.toString("base64")}`, reply:"できました。元画像をそのまま維持して、指定された文字だけを追加しました。" });
  } catch (error) {
