...
export default async function handler(req, res) {
  if (req.method === 'POST' && req.body) {
    ...
    const forceLogoDesign = mode === 'text' || mode === 'logo' || mode === 'text-logo';
    if (forceLogoDesign || DESIGN_RE.test(message)) {
      const body = { ...req.body, message: `${message}${PREMIUM_DIRECTIVE}` };
