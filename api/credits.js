...
async function callRpc(name, userId, extra = {}) {
  const url = process.env.SUPABASE_URL, key = process.env.SUPABASE_SERVICE_ROLE_KEY;
  if (!url || !key) throw new Error('SUPABASE_NOT_CONFIGURED');
  const r = await fetch(`${url}/rest/v1/rpc/${name}`, { method: 'POST', headers: { apikey: key, Authorization: `Bearer ${key}`, 'Content-Type': 'application/json' }, body: JSON.stringify({ p_user_id: userId, ...extra }) });
...
export default async function handler(req, res) {
  ...
  try {
    if (req.method === 'GET') {
      const r = await fetch(`${process.env.SUPABASE_URL}/rest/v1/profiles?id=eq.${encodeURIComponent(user.id)}&select=plan,credits,purchased_credits`, { headers: { apikey: process.env.SUPABASE_SERVICE_ROLE_KEY, Authorization: `Bearer ${process.env.SUPABASE_SERVICE_ROLE_KEY}` } });
      if (!r.ok) throw new Error(await r.text());
    ...
    if (action === 'consume') {
      ...
      if (!result?.success) return json(res,402,{error:'NO_CREDITS',message:'クレジットがありません。'});
      return json(res,200,result);
    }
    if (action === 'refund') return json(res,200,await callRpc('iconia_refund_credit',user.id));
    if (action === 'grant_ad') return json(res,200,await callRpc('iconia_grant_ad_credit',user.id));
